...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        # tpfp = tpfp_func(cls_dets[0], cls_gts[0], cls_gts_ignore[0], iou_thr, area_ranges)
    ...
    print_map_summary(
        mean_ap, eval_results, level, dataset, area_ranges, logger=logger)
...
def print_map_summary(mean_ap,
    ...
    header = ['level', 'class', 'gts', 'dets', 'recall', 'ap']
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        for j in range(num_classes):
            row_data = [
                str(level), label_names[j], num_gts[i, j], results[j]['num_dets'],
...
if __name__ == '__main__':
    import mmcv
    det_results = mmcv.load('data/det_results.pkl')
    targets = mmcv.load('data/test_annos.pkl')

    results_dict = {}
    annotations = []
    num_classes = 2
    threshold = 0.27
    for res_list in det_results:
        for level, res in enumerate(res_list):
