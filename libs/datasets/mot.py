...
class MOTDataset(Dataset):
        ...
        for img_dir in os.listdir(data_root):
            ...
            for line in lines:
                ...
                if int(valid) == 1:
                    ...
                    label_encode = self.multi_dense_to_one_hot(np.array([int(label)]), self.num_classes+1)
                    video_anno[video_name]['anno'][int(frame_id)]['labels'].append(label_encode)
        ...
        for video_name in video_anno.keys():
            for frame_id in video_anno[video_name]['anno'].keys():
                ...
                cur_anno_info = {
                    ...
                    'labels': np.vstack(anno['labels']).reshape(-1, self.num_classes),
    ...
    def __getitem__(self, index):
        ...
        labels = torch.from_numpy(labels.reshape(-1, self.num_classes).astype(np.int64))
        vis_ratios = torch.from_numpy(vis_ratios.reshape(-1, ).astype(np.float32))
        target = dict(
            boxes=boxes,
            labels=labels,
        ...
        next_labels = torch.from_numpy(next_labels.reshape(-1, self.num_classes).astype(np.int64))
        ...
        out_target = dict(
            boxes=target['boxes'],
            next_boxes=next_target['boxes'],
            labels=target['labels'],
            next_labels=next_target['labels'],
