...
class DeformableMatchTrack(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        self.ref_id_projector = projection_MLP(hidden_dim, hidden_dim, emb_dim)
        self.ref_id_predictor = prediction_MLP(emb_dim, emb_dim, emb_dim)
    ...
    def forward(self, samples: NestedTensor, references=None):
        ...
        if references is not None and match_hs is not None:
            ...
            for lvl in range(match_hs.shape[0]):
                ...
                if lvl == match_hs.shape[0] - 1:
                    if len(match_hs[lvl][0]) == 1:
                        match_hs_ref = torch.cat([match_hs, match_hs], dim=2)
                        ref_id_embed = self.ref_id_projector(match_hs_ref[lvl].flatten(0, 1))
                        ref_id_feature = ref_id_embed[0].clone().reshape(
                            match_hs_ref[lvl].shape[0], match_hs_ref[lvl].shape[1]// 2, -1)
                        ref_id_embed = self.ref_id_predictor(ref_id_embed)[0].reshape(
                            match_hs_ref[lvl].shape[0], match_hs_ref[lvl].shape[1]//2, -1)
                    else:
                        ref_id_embed = self.ref_id_projector(match_hs[lvl].flatten(0, 1))
                        ref_id_feature = ref_id_embed.clone().reshape(
                            match_hs[lvl].shape[0], match_hs[lvl].shape[1], -1)
                        ref_id_embed = self.ref_id_predictor(ref_id_embed).reshape(
                            match_hs[lvl].shape[0], match_hs[lvl].shape[1], -1)
        ...
        if references is not None and match_hs is not None:
            out['ref_outputs'] = {'ref_coords': ref_coords[-1],
               'ref_id_preds': ref_id_embed, 'ref_id_features': ref_id_feature}
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, log=True, is_next=False):
        ...
        if 'id_preds' in outputs and self.references is not None and self.ori_indices is not None and self.ref_num_boxes is not None:
            ...
            if len(valid_idx) == 0:
                ...
                # prev_id_preds = torch.cat([r["prev_id_preds"][idx] for r, (_, idx) in zip(
                #     self.references, id_indices)])
    ...
    def loss_match(self, outputs, targets, indices, num_boxes, log=True, is_next=False):
        ...
        # id
        prev_id_features = torch.cat([r['ref_features'] for r in self.references], dim=0)
        if len(prev_id_features) == 0:
            losses['match_loss_ids'] = torch.Tensor([0.]).mean().to(device)
        else:
            id_preds = outputs['ref_outputs']['ref_id_preds'].flatten(0, 1)
            assert len(id_preds.shape) == 2
            loss_ids = - F.cosine_similarity(id_preds, prev_id_features.detach(), dim=1).sum() / self.ref_num_boxes
            # loss_ids = sigmoid_focal_loss(outputs_src_id_logits, target_ids_onehot, reduction='sum', alpha=self.focal_alpha, gamma=2) / max(1, outputs_src_id_logits.shape[0])
...
class PostProcess(nn.Module):
    ...
    def __init__(self):
        ...
        self.refer_matcher = MatchTrackMatcher(det_thr=0.38, cost_limit=0.4)
