...
def eval_seq(cfg, dataloader, data_type, result_filename, save_dir=None, show_image=True,
...
def main(cfg, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
         save_images=False, save_videos=False, show_image=True, logger=None):
    if logger is not None:
    ...
    for seq in seqs:
        ...
        # dataloader = datasets.LoadImages(osp.join(data_root, seq, 'img1'), cfg.img_size)
        # result_filename = os.path.join(result_root, '{}.txt'.format(seq))
        # meta_info = open(os.path.join(data_root, seq, 'seqinfo.ini')).read()
        # frame_rate = int(meta_info[meta_info.find('frameRate') + 10:meta_info.find('\nseqLength')])
        nf, ta, tc = eval_seq(cfg, dataloader, data_type, result_filename,
