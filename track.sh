PARTITION=VA
srun --partition=$PARTITION --mpi=pmi2 -n 8 --gres=gpu:8 --ntasks-per-node=8 --job-name=EVAL --kill-on-bad-exit=1 python3 tools/match_track_simple.py --cfg configs/deformable_track.yaml \
