PARTITION=Test
srun --partition=$PARTITION --mpi=pmi2 -n 1 --gres=gpu:1 --ntasks-per-node=8 --job-name=EVAL --kill-on-bad-exit=1 -x BJ-IDC1-10-10-16-[53,54,58,59,62,64,68,86,85] python3 tools/offset_track_KITTI.py --cfg configs/deformable_macthtrack_kitti.yaml \
     MODEL.RESUME_PATH output/matchtrack_kitti_DeformableMatchTrack_epoch043_checkpoint.pth
