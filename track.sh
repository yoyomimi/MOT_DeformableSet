...
srun --partition=$PARTITION --mpi=pmi2 -n 1 --gres=gpu:1 --ntasks-per-node=8 --job-name=EVAL --kill-on-bad-exit=1 python tools/offset_track.py --cfg configs/deformable_track.yaml \
     MODEL.RESUME_PATH output/matchtrack_chnomatch_layer6_joint_from20_DeformableMatchTrack_epoch005_checkpoint.pth
